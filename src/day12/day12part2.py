...
def main(args):
    ...
    startPoints = [tuple(coord) for coord in np.argwhere((heightmap == 'a') | (heightmap == 'S')).tolist()]
    end = np.nonzero(heightmap == 'E')
    print(min(distance(heightmap, startPoint, end) for startPoint in startPoints))
